...
export default function ClientIndex({ auth, clients }: Props) {
    ...
    const handlePrint = (client: Client) => {
        ...
        // Delay 100ms agar React sempat render data ke hidden div sebelum print dipanggil
        setTimeout(() => {
            window.print();
            // setClientToPrint(null); // (Opsional: Bersihkan state setelah print)
        }, 150);
    };
        ...
        <AppLayout breadcrumbs={[{ title: 'Dashboard', href: '/dashboard' }, { title: 'Data Klien', href: '/clients' }]}>
            ...
            <style>{`
                @media print {
                    ...
                    #print-client-card, #print-client-card * { visibility: visible !important; color: #000000 !important; }
                    #print-client-card { position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; margin: 0 !important; padding: 0 !important; background-color: #ffffff !important; }

                    /* Styling khusus tabel print */
                    #print-client-card table { border-collapse: collapse !important; width: 100% !important; }
                    #print-client-card th, #print-client-card td { padding: 12px 8px !important; border-bottom: 1px solid #000000 !important; text-align: left !important; }
                    #print-client-card th { width: 35% !important; }
                    ::-webkit-scrollbar { display: none !important; }
                }
            `}</style>

            {/* --- PRINT AREA (DISEMBUNYIKAN DI LAYAR, MUNCUL SAAT CETAK) --- */}
            <div id="print-client-card" className="hidden print:block font-serif">
                {clientToPrint && (
                    <div className="border-[3px] border-black p-8 rounded-xl m-4">
                        <div className="text-center border-b-[3px] border-black pb-6 mb-6">
                            <h1 className="text-2xl font-black uppercase tracking-[0.2em] mb-1">Data Profil Klien</h1>
                            <p className="text-sm font-bold uppercase tracking-widest text-slate-800">Kantor Notaris & PPAT</p>
                        </div>

                        <table>
                            <tbody>
                                <tr>
                                    <th className="font-bold uppercase tracking-wider text-xs">Nama Lengkap / Badan</th>
                                    <td className="font-bold text-lg">{clientToPrint.name}</td>
                                </tr>
                                <tr>
                                    <th className="font-bold uppercase tracking-wider text-xs">Tipe Klien</th>
                                    <td className="font-bold uppercase">{clientToPrint.type.replace('_', ' ')}</td>
                                </tr>
                                <tr>
                                    <th className="font-bold uppercase tracking-wider text-xs">NIK / NPWP</th>
                                    <td className="font-mono font-bold tracking-widest">{clientToPrint.nik_or_npwp}</td>
                                </tr>
                                <tr>
                                    <th className="font-bold uppercase tracking-wider text-xs">No. Handphone</th>
                                    <td className="font-bold">{clientToPrint.phone}</td>
                                </tr>
                                <tr>
                                    <th className="font-bold uppercase tracking-wider text-xs">Email</th>
                                    <td className="font-bold">{clientToPrint.email || '-'}</td>
                                </tr>
                                <tr>
                                    <th className="font-bold uppercase tracking-wider text-xs align-top pt-4">Alamat Domisili</th>
                                    <td className="font-bold pt-4">{clientToPrint.address}</td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        {/* <div className="mt-16 flex justify-end">
                            <div className="text-center w-56">
                                <p className="text-xs mb-20 font-medium">Dicetak pada: {new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
            ...
            <div className="min-h-screen bg-slate-50 dark:bg-black p-6 lg:p-8 font-sans print:hidden">
                {/* --- HEADER SECTION --- */}
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="pl-10 pr-4 py-3 w-full sm:w-72 bg-white dark:bg-neutral-800 border border-slate-200 dark:border-neutral-700 rounded-2xl text-sm focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 transition-all shadow-sm hover:border-slate-300 outline-none text-slate-800 dark:text-white"
                            />
                        </div>
                ...
                {/* --- TABLE CARD --- */}
                <div className="bg-white dark:bg-neutral-800 rounded-[2rem] shadow-2xl shadow-slate-200/50 dark:shadow-none border dark:border-neutral-700 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y dark:divide-neutral-700">
                            ...
                            <tbody className="bg-white dark:bg-neutral-800 divide-y dark:divide-neutral-700">
                                {filteredClients.length === 0 ? (
                                    ...
                                    filteredClients.map((client) => (
                                        <tr key={client.id} className="group hover:bg-slate-50 dark:hover:bg-neutral-700/50 transition-all duration-300">
                                            <td className="px-8 py-5 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className={`h-12 w-12 rounded-2xl flex items-center justify-center text-sm font-black text-white shadow-lg transition-transform group-hover:scale-110 group-hover:rotate-3
                                                        ${client.type === 'perorangan' ? 'bg-gradient-to-br from-cyan-500 to-blue-600 shadow-cyan-200/50'
                                                        : client.type === 'badan_hukum' ? 'bg-gradient-to-br from-indigo-500 to-purple-600 shadow-indigo-200/50'
                                                        : 'bg-gradient-to-br from-amber-500 to-orange-500 shadow-amber-200/50'}`
                                                    }>
                                                        {getInitials(client.name)}
                                                    </div>
                                                    <div className="ml-4">
                                                        <div className="text-sm font-black text-black dark:text-white group-hover:text-cyan-600 transition-colors">
                                                            {client.name}
                                                        </div>
                                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border mt-1 ${
                                                            client.type === 'perorangan' ? 'bg-cyan-50 text-cyan-700 border-cyan-100 dark:bg-cyan-500/10 dark:text-cyan-400 dark:border-cyan-500/20'
                                                            : client.type === 'badan_hukum' ? 'bg-indigo-50 text-indigo-700 border-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20'
                                                            : 'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20'
                                                        }`}>
                                                            {client.type === 'perorangan' ? 'Perorangan' : client.type === 'badan_hukum' ? 'Badan Hukum' : 'Kelompok'}
                                                        </span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5 whitespace-nowrap">
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest">
                                                        {client.type === 'perorangan' ? 'NIK' : client.type === 'badan_hukum' ? 'NPWP' : 'NIK / NPWP'}
                                                    </span>
                                                    <span className="text-sm text-slate-700 dark:text-slate-300 font-mono font-bold tracking-tighter bg-slate-100 dark:bg-neutral-900 px-3 py-1 rounded-xl border border-slate-200 dark:border-neutral-700 w-fit">
                                                        {client.nik_or_npwp}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5 whitespace-nowrap">
                                                <div className="flex flex-col gap-1.5">
                                                    <div className="flex items-center gap-2 text-sm font-bold text-slate-600 dark:text-slate-300">
                                                        <div className="w-6 h-6 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 flex items-center justify-center">
                                                    ...
                                                    {client.email && (
                                                        <div className="flex items-center gap-2 text-xs font-medium text-slate-400">
                                                            <div className="w-6 h-6 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 flex items-center justify-center pl-0.5">
                                            ...
                                            <td className="px-8 py-5 whitespace-nowrap text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0">
                                                    <button
                                                        ...
                                                        className="p-2.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-2xl transition-all"
                                                        title="Cetak Data Klien"
                                                    >
                                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                                    </button>
                                                    <button
                                                        onClick={() => openEditModal(client)}
                                                        className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-2xl transition-all"
                                                        title="Edit Data"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-2.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-2xl transition-all"
                                                        title="Hapus Data"
                                                    >
                    ...
                    <div className="bg-slate-50/80 dark:bg-neutral-900/50 px-8 py-4 border-t border-slate-100 dark:border-neutral-700">
                        <p className="text-[11px] font-bold text-slate-500 text-center sm:text-left uppercase tracking-widest">
                            Total: <span className="text-slate-900 dark:text-white">{filteredClients.length} Klien Terdaftar</span>
                        </p>
                    </div>
                </div>
            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select value={data.type} onChange={(e) => setData('type', e.target.value)} className={`${inputClasses} cursor-pointer`}>
                                        ...
                                        {/* TAMBAHAN OPSI KELOMPOK */}
                                        <option value="kelompok">👥 Kelompok / Bersama</option>
                                    </select>
                                </div>
                                <div>
                                    {/* LABEL BERUBAH DINAMIS SESUAI TIPE */}
                                    <label className={labelClasses}>
                                        {data.type === 'perorangan' ? 'NIK (Identitas)' : data.type === 'badan_hukum' ? 'NPWP Perusahaan' : 'NIK Perwakilan / NPWP'}
                                    </label>
                                    <input type="text" value={data.nik_or_npwp} onChange={(e) => setData('nik_or_npwp', e.target.value)} className={`${inputClasses} font-mono tracking-widest`} placeholder="Angka Identitas..." required />
